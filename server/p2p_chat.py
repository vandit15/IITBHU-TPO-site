...
class P2pChat(tk.Frame):
    def __init__(self, master=None):
        ...
        name=s+".xml"
        bool=os.path.isfile(str(os.path.dirname(__file__))+'\\'+'\\'+name)
        if bool==False:
            print "new"
            rr=ET.Element("root")
            ET.SubElement(rr,"thread",name="Message Box").text="Messages"
            t=ET.ElementTree(rr)
            t.write(name)    
        self.tree = ET.parse(str(os.path.dirname(__file__))+'\\'+'\\'+name)
        self.root=self.tree.getroot()
    def allow(self,event=None):
        to=""
        def send():
            to=e1.get()
            ...
            ser.starttls()
            ser.ehlo()
            ser.login('miramiranomi07@gmail.com','devilfruit')
            message=MIMEMultipart()
            part=MIMEText("Ip Address :%s,Port %s\nTo chat open chat box by clicking on the address 127.0.0.1:8000/tpo/chat"%(str(self.chat.ip),str(DEFAULT_PORT)),'plain')
            message.attach(part)
            ser.sendmail('miramiranomi07@gmail',to,message.as_string())
            tkMessageBox.showinfo("MAIL SENT",'Mail has been sent')
        r=tk.Tk()
        l=Label(r,text='Enter Email')
        e1=Entry(r)
    ...
    def createWidgets(self):
        ...
        sendport_btn = tk.Button(msg_entry_frame)
        sendport_btn["text"] = "Allow Connection"
        sendport_btn["command"] = self.allow
        sendport_btn.pack(side=tk.RIGHT)
    ...
    def display_new_msg(self):
        if self.chat is not None:   
            ...
            for msg in msgs:
                ...
                if 1:
                    try:
                        self.root[0].text+=msg[:-1]+"\n"
                        s=str(self.chat._port)
                        name=s+".xml"
                        self.tree.write(str(os.path.dirname(__file__))+'\\'+'\\'+name)
                        obj=ET.parse(str(os.path.dirname(__file__))+'\\'+'\\'+name)
                        r=obj.getroot()
                        r[0]=msg[:-1]+"\n"
                    except Exception:
                        print "fail"
